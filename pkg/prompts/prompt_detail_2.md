...
> - **[Minor]:** 修正が望ましい問題（コード品質、可読性、命名規則）。
...
* **要素: 行番号**
    * **基準:** 変更後のコード（`+` で始まる行）の正確な行番号。
* **要素: 対象コード**
    * **基準:** 指摘箇所のコード断片を引用（行番号のズレ防止のため）。
* **要素: 問題点**
    * **基準:** 観点（バグ、セキュリティ、可読性など）を明記し、問題の核心を記述する。
* **要素: 修正案**
    * **基準:** 影響を最小限に抑え、具体的なコード変更案をコードブロックで示す。
