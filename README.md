...
本ライブラリは、**CLIツール**や**Webアプリケーション**の共通基盤として設計されています。Git操作、AI通信、プロンプト生成、HTML変換といった「ビジネスロジック」に加え、CLIやWebアプリで共通利用可能な結果保存のアダプター（Publisher）を提供します。これにより、利用側はインフラの実装詳細を意識することなく、レビュー結果の生成から公開までを一貫して行えます。
...
| **ストレージ操作** | **`go-remote-io`** (`publisher`層) | GCS等のクラウドストレージへのアップロード処理を抽象化し、CLIとWebアプリで接続処理（Factory）を共通化するために使用します。 |
...
* **共通化された保存ロジック:** レビュー結果（HTML）をクラウドストレージ（GCS等）へ保存する処理を **Publisher** パターンとして提供しています。
...
│   │   └── html\_runner.go    \# go-text-formatを使用したHTML変換の実装
│   │
│   ├── publisher \# 結果の出力・公開層 (Storage Adapter)
│   │   └── gcs.go            \# GCSへのHTML保存とヘッダー生成の実装
│   │
